...
import { Wallet, Home, NotebookPen, Users, UserRound, PlusCircle, ChevronDown, ChevronRight, Menu } from "lucide-react";
...
export default function UserLayout() {
  ...
  const [addressbookOpen, setAddressbookOpen] = useState(true);
          ...
          <nav className='p-2 space-y-1'>
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`absolute left-0 h-5 rounded-r ${isActive ? `w-[3px] ${isOpen ? "bg-primary" : "bg-white"}` : "w-0"}`} />
                  <PlusCircle className={`h-5 w-5 ${isActive ? (isOpen ? "text-primary" : "text-white") : isOpen ? "text-ink/60 group-hover:text-primary" : "text-white/80 group-hover:text-white"}`} />
                  {isOpen && <span>New Consignment</span>}
                </>
              )}
            </NavLink>
            ...
            {isOpen && (
              ...
              >
                ...
                {addressbookOpen ? <ChevronDown className='h-4 w-4' /> : <ChevronRight className='h-4 w-4' />}
              </button>
            )}
            <NavLink
              to='senders'
                ...
                `${linkBase} ${
                  isActive
                    ...
                    : isOpen
                      ? "text-ink/80"
                      : "text-white/90"
                } ${addressbookOpen ? "block" : "hidden"}`
              }
            >
              {({ isActive }) => (
                <>
                  <span className={`absolute left-0 h-5 rounded-r ${isActive ? `w-[3px] ${isOpen ? "bg-primary" : "bg-white"}` : "w-0"}`} />
                  <UserRound className={`h-5 w-5 ${isActive ? (isOpen ? "text-primary" : "text-white") : isOpen ? "text-ink/60 group-hover:text-primary" : "text-white/80 group-hover:text-white"}`} />
                  {isOpen && <span>Manage Senders</span>}
                </>
              )}
            </NavLink>
            <NavLink
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`absolute left-0 h-5 rounded-r ${isActive ? `w-[3px] ${isOpen ? "bg-primary" : "bg-white"}` : "w-0"}`} />
                  <Users className={`h-5 w-5 ${isActive ? (isOpen ? "text-primary" : "text-white") : isOpen ? "text-ink/60 group-hover:text-primary" : "text-white/80 group-hover:text-white"}`} />
                  {isOpen && <span>Manage Receivers</span>}
                </>
              )}
            </NavLink>
            ...
            {isOpen && (
              ...
              >
                <span>Wallet</span>
                {walletOpen ? <ChevronDown className='h-4 w-4' /> : <ChevronRight className='h-4 w-4' />}
              </button>
            )}
            <NavLink
              to='wallet/top-up'
                ...
                `${linkBase} ${
                  isActive
                    ...
                    : isOpen
                      ? "text-ink/80"
                      : "text-white/90"
                } ${walletOpen ? "block" : "hidden"}`
              }
            >
              {({ isActive }) => (
                <>
                  <span className={`absolute left-0 h-5 rounded-r ${isActive ? `w-[3px] ${isOpen ? "bg-primary" : "bg-white"}` : "w-0"}`} />
                  <Wallet className={`h-5 w-5 ${isActive ? (isOpen ? "text-primary" : "text-white") : isOpen ? "text-ink/60 group-hover:text-primary" : "text-white/80 group-hover:text-white"}`} />
